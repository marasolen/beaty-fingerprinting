...
const setupSingleRowPetri = (row) => {
    ...
    const codeToStops = (code) => {
        const stops = [];
        stops.push([code.substring(0, 1), "0%"]);
        stops.push([code.substring(0, 1), "20%"]);
        stops.push([code.substring(1, 2), "40%"]);
        stops.push([code.substring(1, 2), "60%"]);
        stops.push([code.substring(2, 3), "80%"]);
        stops.push([code.substring(2, 3), "100%"]);
        return stops;
    };

    defs.selectAll("stop")
        .data(d => codeToStops(d.id))
        .join("stop")
        .attr("offset", d => d[1])
